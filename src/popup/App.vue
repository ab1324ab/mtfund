<template>
  <div id="app" class="container" ref="app" :class="[containerClass]" :style="[zoom, opacity]">
    <div ref="posmask" style="position: fixed;top: 0;left: 0;height: 100%;width: 100%;"></div>
    <div :style="grayscale">
      <div class="tab-row" v-if="isGetStorage" :element-loading-background="darkMode ? 'rgba(0, 0, 0, 0.9)' : 'rgba(255, 255, 255, 0.9)'" :style="seciList.length > 0 ? { minHeight: '55px' } : {}">
        <div v-for="(el, index) in indFundData" :draggable="isEdit" class="tab-col indFund" :class="drag" :key="el.f12" @click.stop="!isEdit && indDetail(el)"
          ...
          <el-card class="box-card" ref="exponent">
            <div slot="header" class="clearfix">
              ...
              <el-button v-if="isEdit" style="float: right; padding: 1px 2px;" type="text" @click="dltIndFund(index)">✖</el-button>
            </div>
            <el-row>
              <el-col :xs="8" :sm="8" ref="exponentIcon">
                <svg style="height: 40px;width: 100%;" t="1692266581586" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5913" width="200" height="200">
                  <path d="M422.4 550.4l-21.333333-21.333333-89.6 89.6-29.866667-29.866667L396.8 469.333333l51.2 51.2L631.466667 341.333333l179.2 179.2-29.866667 29.866667-149.333333-149.333333-183.466667 183.466666-25.6-34.133333zM213.333333 810.666667h597.333334v42.666666H170.666667V213.333333h42.666666v597.333334z" fill="#444444" p-id="5914"></path>
                </svg>
              </el-col>
        ...
        <div v-if="isEdit" class="tab-col showAddSeci"> <!--          && indFundData.length < 4-->
          <el-card class="box-card" style="height: 98%;">
            <div v-if="!showAddSeciInput" class="addSeci" @click="() => (showAddSeciInput = true)">
              <svg style="width: 40px;height: 40px" t="1692282076148" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="7038" width="200" height="200">
                <path d="M509.8 939.7c-239.4 0-434.2-194.8-434.2-434.2S270.4 71.3 509.8 71.3 944 266.1 944 505.5 749.2 939.7 509.8 939.7z m0-832.7c-219.7 0-398.5 178.8-398.5 398.5S290.1 904 509.8 904s398.5-178.8 398.5-398.5S729.6 107 509.8 107z" fill="#c0c4cc" p-id="7039"></path>
                <path d="M492 736V275c0-1.1 0.9-2 2-2h31.7c1.1 0 2 0.9 2 2v461c0 1.1-0.9 2-2 2H494c-1.1 0-2-0.9-2-2z" fill="#c0c4cc" p-id="7040"></path>
                <path d="M279.3 487.7h461.1c1.1 0 2 0.9 2 2v31.7c0 1.1-0.9 2-2 2H279.3c-1.1 0-2-0.9-2-2v-31.7c0-1.1 0.9-2 2-2z" fill="#c0c4cc" p-id="7041"></path>
              </svg>
            </div>
            <div v-else style="margin: 6.2px auto;">
              <div style="padding-top:2px">
                <el-select size="mini" :popper-append-to-body="false" v-model="sltSeci" placeholder="请选择">
      ...
      <div v-if="isEdit" class="searchFunds" style="margin-bottom: 5px;">
        <el-card class="box-card" style="height: 100%;">
          ...
          <div style="padding: 2px;">
            <el-row type="flex" justify="center">
              <span style="display: block;width: 140px;line-height: 28px;">添加新基金：</span>
              <!-- <input v-model="fundcode" class="btn" type="text" placeholder="请输入基金代码" /> -->
              <el-select v-model="fundcode" multiple filterable :popper-append-to-body="false" remote size="mini" reserve-keyword @visible-change="selectChange" placeholder="请输入基金名称或编码搜索，部分新发基金或QDII基金可以搜索到，但可能无法获取估值情况" :remote-method="remoteMethod" :loading="loading" style="width:100%;">
              ...
              <input @click="save" class="btn primary" type="button" style="padding: 6px 35px;margin-left: 20px" value="确定"/>
            </el-row>
          </div>
      ...
      <el-tabs v-model="activeName" v-loading="loadingList" type="card" addable size="mini" class="active-funds-tabs" @tab-add="addTabPane" @tab-remove="removeTabPane">
        <el-tab-pane label="全部基金" name="first">
          <div v-if="isGetStorage" :element-loading-background="darkMode ? 'rgba(0, 0, 0, 0.9)' : 'rgba(255, 255, 255, 0.9)'" class="table-row"><!--  style="min-height:160px"-->
            <table ref="table" :class="tableHeight + ' table-container'">
              <thead>
              <tr>
                <th class="align-left">基金名称（{{dataList.length}}）</th>
                <th v-if="isEdit">基金代码</th>
                <th v-if="showGSZ && !isEdit" style="min-width: 70px;">估算净值</th>
                <th v-if="isEdit && (showCostRate || showCost)">成本价格</th>
                <th v-if="isEdit && (showAmount || showGains || showCost || showCostRate)">持有份额</th>
                ...
                <th @click="sortList('costGainsRate')" v-if="!isEdit && showCostRate" class="pointer">
                  持有收率
                ...
                <th @click="sortList('navchgrt')" v-if="!isEdit && showNavchgrt" class="pointer">
                  昨日涨跌
              ...
              <tbody>
              <template v-if="dataList.length > 0" v-for="(el, index) in dataList">
                <tr :key="el.fundcode" :draggable="isEdit" :class="drag" :style="el.trStyle" @dragstart="handleDragStart($event, el)" @dragover.prevent="handleDragOver($event, el)"
                    @dragenter="handleDragEnter($event, el, index)" @dragend="handleDragEnd($event, el)">
                  <el-tooltip :disabled="el.tips == '' || el.tips == null" :effect="darkMode? 'dark':'light'" placement="right"><!-- placement="top-end" :content="el.tips"-->
                    <div slot="content" v-html="el.tips"></div>
                    <td :class="isEdit ? 'fundName-noclick align-left' : 'fundName align-left'" @click.stop="!isEdit && fundDetail(el)"><!-- :title="el.name" -->
                  ...
                  <td v-if="isEdit && (showCostRate || showCost)">
                    <input class="btn num" style="cursor: auto;" placeholder="持仓成本价" v-model="el.cost" @input="changeCost(el, index)" type="text"/>
                  </td>
                  <td v-if="isEdit && (showAmount || showGains || showCost || showCostRate)">
                    <input class="btn num" style="cursor: auto;" placeholder="输入持有份额" v-model="el.num" @input="changeNum(el, index)" type="text"/>
                  </td>
                  <td v-if="showAmount"><!-- style="position: relative;"-->
                    <span v-if="!isEdit">
                      {{parseFloat(el.amount).toLocaleString("zh", {minimumFractionDigits: 2,})}}
                    </span>
                    <input v-else class="btn num" style="cursor: auto;" placeholder="输入持有金额" v-model="el.amount" @input="changeAmount(el, index)" type="text"/>
                  </td>
                  <td v-if="isEdit">
                    <div style="min-width: 66px;">
                      <el-button style="padding: 0" title="持有份额加仓或者卖出" @click="positionIncrdec(el)">
                        <svg style="height: 17px;width: 40px;" t="1686330770451" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1508" width="64" height="64">
                          <path d="M390.948571 288.073143l100.022858 99.986286 100.022857-99.986286a27.611429 27.611429 0 0 1 40.009143 0 27.611429 27.611429 0 0 1 0 40.009143l-108.617143 108.617143v22.820571h97.206857c17.152 0 28.562286 11.446857 28.562286 28.598857 0 17.152-11.410286 28.562286-28.562286 28.562286h-97.206857v45.714286h97.206857c17.152 0 28.562286 11.446857 28.562286 28.598857 0 17.188571-11.410286 28.598857-28.562286 28.598857h-97.206857v62.866286c-5.705143 17.152-17.115429 28.562286-28.525715 28.562285-17.188571 0-28.598857-11.410286-28.598857-28.562285v-62.902858H362.349714c-17.188571 0-28.598857-11.410286-28.598857-28.525714 0-17.188571 11.446857-28.598857 28.598857-28.598857h102.838857v-45.714286H362.422857c-17.188571 0-28.598857-11.446857-28.598857-28.598857 0-17.152 11.446857-28.562286 28.598857-28.562286h102.838857v-17.188571L350.939429 328.082286a27.611429 27.611429 0 0 1 0-40.009143 27.611429 27.611429 0 0 1 40.009142 0z" fill="#333333" p-id="1509"></path>
                          <path d="M493.824 951.113143C242.322286 951.113143 36.571429 745.325714 36.571429 493.824S242.322286 36.571429 493.824 36.571429s457.289143 205.750857 457.289143 457.252571c0 22.857143 0 45.714286-5.741714 74.313143 0 17.152-17.152 28.598857-34.267429 22.857143-17.188571 0-28.598857-17.152-22.893714-34.304 5.741714-22.857143 5.741714-40.009143 5.741714-62.866286 0-222.902857-177.188571-400.091429-400.091429-400.091429-222.939429 0-400.128 182.893714-400.128 405.833143 0 222.902857 177.188571 400.091429 400.091429 400.091429 22.857143 0 51.456 0 74.313143-5.705143 17.152-5.741714 28.598857 5.705143 34.304 22.857143 5.705143 17.152-5.705143 28.562286-22.857143 34.304h-85.76z" fill="#333333" p-id="1510"></path>
                          <path d="M936.082286 825.161143h-285.805715c-17.115429 0-28.562286-11.446857-28.562285-28.598857 0-17.115429 11.446857-28.562286 28.562285-28.562286h285.805715c17.152 0 28.562286 11.446857 28.562285 28.562286 0 17.188571-17.115429 28.598857-28.562285 28.598857z" fill="#F72F39" p-id="1511"></path>
                          <path d="M796.562286 964.644571c-17.115429 0-28.562286-11.410286-28.562286-28.562285v-285.805715c0-17.115429 11.446857-28.562286 28.562286-28.562285 17.188571 0 28.598857 11.446857 28.598857 28.562285v285.805715c0 11.446857-17.152 28.562286-28.598857 28.562285z" fill="#F72F39" p-id="1512"></path>
                        </svg>
                      </el-button>
                      <el-badge type="primary" :value="el.count" style="vertical-align: unset;" class="el-content-primary">
                        <el-button style="padding: 0;margin-left: 0;" title="定投份额" @click="positionFinvest(el)">
                          <svg style="height: 17px;width: 17px;" t="1686331492654" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1671" width="64" height="64">
                            <path d="M493.714286 18.285714c262.582857 0 475.428571 212.845714 475.428571 475.428572a36.571429 36.571429 0 0 1-73.142857 0c0-222.171429-180.114286-402.285714-402.285714-402.285715s-402.285714 180.114286-402.285715 402.285715 180.114286 402.285714 402.285715 402.285714a36.571429 36.571429 0 0 1 0 73.142857c-262.582857 0-475.428571-212.845714-475.428572-475.428571s212.845714-475.428571 475.428572-475.428572z" fill="#333333" p-id="1672"></path>
                            <path d="M274.285714 566.857143a36.571429 36.571429 0 0 1 0-73.142857h182.857143v-182.857143a36.571429 36.571429 0 0 1 73.142857 0v219.428571a36.571429 36.571429 0 0 1-36.571428 36.571429z" fill="#E50738" p-id="1673"></path>
                            <path d="M617.142857 676.571429a171.446857 91.428571 0 1 0 342.893714 0 171.446857 91.428571 0 1 0-342.893714 0Z" fill="#333333" opacity=".397" p-id="1674"></path>
                            <path d="M945.737143 749.714286c9.142857 11.190857 14.262857 23.552 14.262857 36.571428 0 50.468571-76.763429 91.428571-171.446857 91.428572-94.646857 0-171.410286-40.96-171.410286-91.428572 0-13.019429 5.12-25.380571 14.262857-36.571428 26.514286 32.292571 86.893714 54.857143 157.147429 54.857143 70.253714 0 130.633143-22.528 157.110857-54.784z" fill="#333333" opacity=".397" p-id="1675"></path>
                            <path d="M945.737143 859.428571c9.142857 11.190857 14.262857 23.552 14.262857 36.571429 0 50.468571-76.763429 91.428571-171.446857 91.428571-94.646857 0-171.410286-40.96-171.410286-91.428571 0-13.019429 5.12-25.380571 14.262857-36.571429 26.514286 32.292571 86.893714 54.857143 157.147429 54.857143 70.253714 0 130.633143-22.528 157.110857-54.784z" fill="#333333" opacity=".397" p-id="1676"></path>
                          </svg>
                        </el-button>
                      </el-badge>
                    </div>
                  ...
                  <td v-if="showCost" :class="el.costGains >= 0 ? 'up' : 'down'">
                    {{parseFloat(el.costGains).toLocaleString("zh", {minimumFractionDigits: 2,})}}
                  </td>
                  <td v-if="!isEdit && showCostRate" :class="el.costGainsRate >= 0 ? 'up' : 'down'">
                    {{el.cost > 0 ? el.costGainsRate + "%" : ""}}
                  </td>
                  <td v-if="!isEdit" :class="el.gszzl >= 0 ? 'up' : 'down'">{{el.gszzl}}%</td>
                  <td v-if="!isEdit && showNavchgrt" :class="el.navchgrt >= 0 ? 'up' : 'down'" style="white-space: nowrap;">
                  ...
                  <td v-if="!isEdit">
                    {{el.gztime == null ? null : (el.hasReplace ? el.gztime.substr(5, 5) : el.gztime.substr(10))}}
                  </td>
                  ...
                  <td v-if="isEdit">
                    <input @click="dlt(el.fundcode)" class="btn red edit" value="x" type="button"/><!--✖-->
                  </td>
                </tr>
              </template>
              <template v-if="dataList.length <= 0">
                <tr>
                  <td ref="tableColspanAll" style="text-align: center;color: #909399;">
                    <small>无基金数据</small>
                  </td>
                </tr>
              </template>
        ...
        <el-tab-pane v-for="item in groupingMap" :key="item.name" :name="item.name" :closable="item.closable">
          ...
          <div v-if="isGetStorage" :element-loading-background="darkMode ? 'rgba(0, 0, 0, 0.9)' : 'rgba(255, 255, 255, 0.9)'" class="table-row"><!--  style="min-height:160px"-->
            <table :class="tableHeight + ' table-container'">
              <thead>
              <tr>
                <th class="align-left">基金名称（{{item.value.length}}）</th>
                <th v-if="isEdit">基金代码</th>
                <th v-if="showGSZ && !isEdit" style="min-width: 70px;">估算净值</th>
                <th v-if="isEdit && (showCostRate || showCost)">成本价格</th>
                <th v-if="isEdit && (showAmount || showGains || showCost || showCostRate)">持有份额</th>
              ...
              <template v-for="(el, index) in item.value">
                <tr :key="el.fundcode" :draggable="isEdit" :class="drag" :style="el.trStyle" @dragstart="handleDragStart($event, el)" @dragover.prevent="handleDragOver($event, el)"
                  ...
                  <td v-if="isEdit">
                    <input @click="dltGroupFund(el.fundcode,item.name)" class="btn red edit" value="x" type="button"/><!--✖-->
                  </td>
                </tr>
              </template>
              </tbody>
    ...
    <div ref="menuDrag" draggable="true" @dragend="dragEnd" :style="grayscale" style="position: fixed;bottom: 35px;right: 50px;">
      <div ref="input-row" class="input-row">
        ...
        <div v-show="isEdit" class="gear-input-row" ref="gearDarkChange" style="position: absolute;right: 100px;bottom: -110px;" @animationend="handleAnimationEnd">
          <el-card style="padding: 10px 5px 15px;overflow: hidden;">
            <el-row type="flex">
              <el-col :xs="8" :sm="8">
                <el-switch v-model="darkMode" class="switch" @change="changeDarkMode" active-color="#484848" inactive-color="#13ce66" inactive-text="亮色" active-text="暗色"></el-switch>
              </el-col>
              <el-col :xs="16" :sm="16">
                <el-row type="flex">
                  <span class="slider-title">灰度：</span>
                  <el-slider vertical height="100px" class="slider" v-model="grayscaleValue" @change="changeGrayscaleValue" :format-tooltip="formatTooltip"></el-slider>
                  <span class="slider-title">透明：</span>
                  <el-slider vertical height="100px" class="slider" :max="90" v-model="opacityValue" @change="changeOpacityValue" :format-tooltip="formatTooltip"></el-slider>
                </el-row>
              </el-col>
            </el-row>
        ...
        <el-row type="flex">
          <input style="display: none;" ref="closedMarketButton" class="btn btn-drag" v-if="!isDuringDate" type="button" value="大盘休市" @animationend="handleAnimationEnd"/>
          <input style="display: none;" ref="isLiveUpdateButton" class="btn btn-drag" v-if="isDuringDate" type="button" :value="isLiveUpdate ? '暂停更新' : '实时更新'" :title="isLiveUpdate ? '正在实时更新，点击暂停' : '已暂停，点击切换为实时更新'" @click="changeLiveUpdate" @animationend="handleAnimationEnd"/>
          <div style="display: none;" class="refresh btn-drag" ref="displayRefresh" :class="{ isRefresh: isRefresh }" title="手动刷新数据" @click="refresh">
            <i class="el-icon-refresh"></i>
          </div>
        </el-row>
        <input style="display: none;" ref="earningsDetailButton" class="btn btn-drag" type="button" value="更新日志" @click="openEarningsDetail" @animationend="handleAnimationEnd"/>
        <input style="display: none;" ref="marketButton" class="btn btn-drag" type="button" @click="market" value="行情中心" @animationend="handleAnimationEnd"/>
        <input style="display: none;" ref="isEditButton" class="btn btn-drag" type="button" :value="isEdit ? '完成编辑' : '编辑基金'" @click="isEdit = !isEdit" @animationend="handleAnimationEnd"/>
        <input style="display: none;" ref="optionButton" class="btn btn-drag" type="button" value="插件设置" @click="option" @animationend="handleAnimationEnd"/>
        <!--      <input class="btn primary" type="button" title="φ(>ω<*)" value="打赏" @click="reward"/>-->
      </div>
      <div class="input-row" ref="incomeCostGains" style="left: -45px;top: -45px;" v-if="showCost || showGains">
        <el-row>
          <el-badge v-if="showGains" :value="(isNaN(allGains[1]) ? '' :  allGains[1] + '%')" :type="allGains[0] >= 0 ? 'danger': 'success'" :class="allGains[0] >= 0 ? 'el-content-danger' : 'el-content-success'">
            <el-tag :type="allGains[0] >= 0 ? 'danger': 'success'" :title="allGains[0] >= 0 ? 'd=====(￣▽￣*)b 赞一个': '∑(っ°Д°;)っ 大事不好啦'">
              日收益:
              <span style="vertical-align: middle;">
                  {{parseFloat(allGains[0]).toLocaleString('zh', {minimumFractionDigits: 2,})}}
                </span>
            </el-tag>
          </el-badge>
          ...
          <el-badge v-if="showCost" :value="(isNaN(allCostGains[1]) ? '' :  allCostGains[1] + '%')" :type="allCostGains[0] >= 0 ? 'danger': 'success'" :class="allCostGains[0] >= 0 ? 'el-content-danger' : 'el-content-success'">
            <el-tag :type="allCostGains[0] >= 0 ? 'danger': 'success'" :title="allCostGains[0] >= 0 ? 'd=====(￣▽￣*)b 赞一个': '∑(っ°Д°;)っ 大事不好啦'">
              总收益:
              <span style="vertical-align: middle;">
                  {{parseFloat(allCostGains[0]).toLocaleString('zh', {minimumFractionDigits: 2,})}}
                </span>
            </el-tag>
          </el-badge>
        </el-row>
      ...
      <div class="menu-drag" ref="menuDragIconRotate" @click="showmenu">
        <svg style="width: 50px;height: 50px;" t="1692611594555" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1608" width="200" height="200">
          <path d="M840.3 132.7H591.1c-27.3 0-49.5 22.2-49.5 49.5v249.2c0 27.3 22.2 49.5 49.5 49.5h249.2c27.3 0 49.5-22.2 49.5-49.5V182.2c0-27.3-22.2-49.5-49.5-49.5zM589 182.2c0-1.2 0.9-2.1 2.1-2.1h249.2c1.2 0 2.1 0.9 2.1 2.1v249.2c0 1.2-0.9 2.1-2.1 2.1H591.1c-1.2 0-2.1-0.9-2.1-2.1V182.2zM840.3 536.7H591.1c-27.3 0-49.5 22.2-49.5 49.5v249.2c0 27.3 22.2 49.5 49.5 49.5h249.2c27.3 0 49.5-22.2 49.5-49.5V586.2c0-27.3-22.2-49.5-49.5-49.5zM589 586.2c0-1.2 0.9-2.1 2.1-2.1h249.2c1.2 0 2.1 0.9 2.1 2.1v249.2c0 1.2-0.9 2.1-2.1 2.1H591.1c-1.2 0-2.1-0.9-2.1-2.1V586.2zM435.3 536.7H186.1c-27.3 0-49.5 22.2-49.5 49.5v249.2c0 27.3 22.2 49.5 49.5 49.5h249.2c27.3 0 49.5-22.2 49.5-49.5V586.2c0-27.3-22.2-49.5-49.5-49.5zM184 586.2c0-1.2 0.9-2.1 2.1-2.1h249.2c1.2 0 2.1 0.9 2.1 2.1v249.2c0 1.2-0.9 2.1-2.1 2.1H186.1c-1.2 0-2.1-0.9-2.1-2.1V586.2z" fill="#8a8a8a" p-id="1609"></path>
          <path d="M418.6 171.1c-31.5-24.1-69.1-36.9-108.8-36.9-37 0-72.5 11.2-102.7 32.4-29.5 20.7-51.9 49.4-64.7 82.9-0.7 1.7-1.1 3.5-1.4 5.4-13 37.7-13.2 79.2-0.3 116.9 0.3 1.3 0.6 2.6 1.1 3.9 16.1 43.2 49.2 79.4 90.8 99.4 1 0.6 1.9 1.1 2.8 1.5 23.6 10.8 48.7 16.3 74.6 16.3 10.2 0 20.4-0.9 30.5-2.6 36.7-6.3 70-23.5 96.4-49.8 33.8-33.8 52.4-78.8 52.4-126.7 0-47.9-18.6-92.9-52.4-126.7-5.7-5.7-11.9-11.1-18.3-16z m20.7 141.2c0.3 34.7-13.1 67.5-37.6 92.2-24.5 24.7-57.1 38.3-91.8 38.3-49.6 0-95.4-29-116.9-73.9-0.3-0.8-0.6-1.6-0.9-2.3-21.7-48.6-11.1-106.5 26.3-144.2 42.4-42.7 110.2-50.3 161.1-18.1 37.1 24 59.4 64.3 59.8 108z" fill="#8a8a8a" p-id="1610"></path>
        </svg>
      </div>
    </div>
    <market ref="marketShadow" :style="grayscale" :darkMode="darkMode" @close="closeCharts"></market>
    <ind-detail ref="indDetail" :style="grayscale" @close="closeCharts" :darkMode="darkMode"></ind-detail>
    <fund-detail ref="fundDetail" :style="grayscale" @close="closeCharts" :fund="sltFund" :darkMode="darkMode"></fund-detail>
    <!--    <reward :style="grayscale" @close="rewardShadow = false" ref="reward"></reward>-->
    <incrdec ref="positionIncrdec" :style="grayscale" :darkMode="darkMode" @passtofather="pidecfun" @close="positionIncrdecShow = false" :fund="pisFund"></incrdec>
    <finvest ref="positionFinvest" :style="grayscale" :darkMode="darkMode" @passtofather="pidecfun" @close="positionFinvestShow = false" :fund="pisFund"></finvest>
    <earnings-detail ref="earningsDetail" :style="grayscale" @close="closeEarningsDetail" :darkMode="darkMode" :top="30"></earnings-detail>
    <groupings ref="groupings" :style="grayscale" @close="closeGroupings" @refresh="refresh" :darkMode="darkMode" :dataList="dataList" :top="30"></groupings>

    <change-log ref="changelog" :style="grayscale" @close="closeChangelog" :darkMode="darkMode" :top="30"></change-log>
  </div>
</template>
...
export default {
  ...
  computed: {
    allGains() {
      ...
      let dataList = this.activeName !== "first" && groupingMap != null ? groupingMap.value : this.dataList;
    ...
    containerClass() {
      ...
      let tablist = [this.showAmount, this.showGains, this.showCost, this.showCostRate, this.showNavchgrt, this.showGSZ,];
      let num = 0;
      tablist.forEach((val) => {
        if (val) num++;
      });
  ...
  methods: {
    ...
    init() {
      chrome.storage.sync.get(
          ["RealtimeFundcode", "RealtimeIndcode", "fundListM", "showAmount", "showGains", "fundList",
            "seciList", "darkMode", "normalFontSize", "isLiveUpdate", "showCost", "showCostRate", "showNavchgrt",
            "showGSZ", "version", "showBadge", "BadgeContent", "userId", "grayscaleValue", "opacityValue", "sortTypeObj",
            "equityUp", "equityDown", "amplitudeUp", "amplitudeDown", "peakedTips", "finallyTips",
            "day5AverageUp", "day5AverageDown", "day10AverageUp", "day10AverageDown", "day20AverageUp", "day20AverageDown", "averageCrossover",
            "groupingArr2"], (res) => {
            chrome.storage.local.get(["monitorFund4", "averageMa4"], les => {
              ...
              this.day5AverageDown = res.day5AverageDown ? res.day5AverageDown : false;
              this.day10AverageUp = res.day10AverageUp ? res.day10AverageUp : false;
              this.day10AverageDown = res.day10AverageDown ? res.day10AverageDown : false;
              this.day20AverageUp = res.day20AverageUp ? res.day20AverageUp : false;
              this.day20AverageDown = res.day20AverageDown ? res.day20AverageDown : false;
              this.averageCrossover = res.averageCrossover ? res.averageCrossover : false;
    ...
    remoteMethod(query) {
      if (query !== "") {
        this.loading = true;
        let url = "https://fundsuggest.eastmoney.com/FundSearch/api/FundSearchAPI.ashx?&m=9&key=" + query + "&_=" + new Date().getTime();
    ...
    sortList(type) {
      ...
      if (this.sortType[type] === "none") {
        ...
        for (let grouping in this.groupingMap) {
          this.groupingMap[grouping].value = this.groupingMap[grouping].value.sort(this.compare(type, this.sortType[type]));
        }
      }
      this.sortTypeObj = {
        name: type,
    ...
    getData(type) {
      ...
      this.$axios.post("https://api.fund.eastmoney.com/favor/GetFundsInfo", {fcodes: fundlist}).then((res) => {
        chrome.runtime.sendMessage({type: "DuringWeekHolidayDate"}, (response) => {
          ...
          let groupingMap = {};
          if (data == null) return;
          let datestr = formatDateTime(new Date(Number(res.data.Expansion.bjTime) * 1000), 'yyyy-MM-dd hh:mm');
          data.forEach((val) => {
            if (this.fundListM.filter((item) => item.code === val.FCODE).length === 0) return;
            ...
            let data = {
              ...
              navchgrt: monitorFund != null ? monitorFund[0].JZZZL : val.RZDF,
              gztime: datestr,
              ftype: val.FTYPE,
            };
            if (data.ftype === "QDII" && data.gszzl === "") data.navchgrt = monitorFund[1].JZZZL;
            if (
            ...
            let slt = this.fundListM.filter((item) => item.code === data.fundcode)[0];
            ...
            if (slt != null && slt.fvest != null && slt.fvest.cvt) {// 定投角标
              data.count = slt.fvest.count == null ? 0 : slt.fvest.count;
            }
            if (data.fundcode === this.RealtimeFundcode) if (this.showBadge === 1) if (this.BadgeContent === 1) {
              chrome.runtime.sendMessage({type: "refreshBadge", data: data,});
            }
            if (monitorFund != null && averageMa != null) {
              ...
              if (this.equityUp.show) {
                ...
                var upfund = monitorFund.filter(item => {
                  if (item.JZZZL >= 0 && checkup) return true; else return checkup = false;
                })
                var upGszzl = upfund.reduce((prev, cur) => prev + parseFloat(cur.JZZZL), 0);
                // console.info("上涨" + upfund.length + "日，共上涨 " + upGszzl.toFixed(2) + " " + data.name);
                if (upfund.length >= this.equityUp.numeric)
                  this.monitorRemindMessage(data, rgbaList, "rgba(245, 108, 108, 0.3)", "该基金已经连续上涨 " + upfund.length + " 日，共上涨 " + upGszzl.toFixed(2) + " %");
              }
              if (this.equityDown.show) {
                var checkdown = true;
                var downfund = monitorFund.filter(item => {
                  if (item.JZZZL < 0 && checkdown) return true; else return checkdown = false;
                })
                var downGszzl = downfund.reduce((prev, cur) => prev + parseFloat(cur.JZZZL), 0);
                // console.info("下跌" + upfund.length + "日，共下跌 " + upGszzl.toFixed(2) + " " + data.name);
                if (downfund.length >= this.equityDown.numeric)
                  this.monitorRemindMessage(data, rgbaList, "rgba(78, 182, 27, 0.3)", "该基金已经连续下跌 " + downfund.length + " 日，共下跌 " + downGszzl.toFixed(2) + " %");
              }
              let minFundVal = Math.min.apply(Math, monitorFund.map(item => item.DWJZ));
              let maxFundVal = Math.max.apply(Math, monitorFund.map(item => item.DWJZ));
              if (this.amplitudeUp.show && minFundVal != null) {
                var upche = ((parseFloat(data.gsz) - minFundVal) / parseFloat(data.gsz) * 100).toFixed(2);
                ...
                if (upche >= this.amplitudeUp.numeric)
                  this.monitorRemindMessage(data, rgbaList, "rgba(245, 108, 108, 0.5)", "累计涨幅 " + upche + " %，超过设定" + this.amplitudeUp.numeric + " %");
              }
              if (this.amplitudeDown.show && maxFundVal != null) {
                var downche = ((parseFloat(data.gsz) - maxFundVal) / parseFloat(data.gsz) * 100).toFixed(2);
                ...
                if (-this.amplitudeDown.numeric > downche)
                  this.monitorRemindMessage(data, rgbaList, "rgba(78, 182, 27, 0.5)", "累计跌幅 " + downche + " %，跌出设定" + this.amplitudeDown.numeric + " %");
              }
              let ma5day, ma10day, ma20day, ma5averageEnd, ma10averageEnd, ma20averageEnd;
              ...
              if (this.day5AverageUp && data.dwjz < ma5day && ma5day < data.gsz) this.monitorRemindMessage(data, rgbaList, "rgba(94,137,255,0.5)", "今日" + averMsg + "05日平均线");
              if (this.day5AverageDown && data.dwjz > ma5day && ma5day > data.gsz) this.monitorRemindMessage(data, rgbaList, "rgba(94,137,255,0.5)", "今日" + averMsg + "05日平均线");
              if (this.day10AverageUp && data.dwjz < ma10day && ma10day < data.gsz) this.monitorRemindMessage(data, rgbaList, "rgba(230,178,63,0.5)", "今日" + averMsg + "10日平均线");
              if (this.day10AverageDown && data.dwjz > ma10day && ma10day > data.gsz) this.monitorRemindMessage(data, rgbaList, "rgba(230,178,63,0.5)", "今日" + averMsg + "10日平均线");
              if (this.day20AverageUp && data.dwjz < ma20day && ma20day < data.gsz) this.monitorRemindMessage(data, rgbaList, "rgba(255,83,255,0.5)", "今日" + averMsg + "20日平均线")
              if (this.day20AverageDown && data.dwjz > ma20day && ma20day > data.gsz) this.monitorRemindMessage(data, rgbaList, "rgba(255,83,255,0.5)", "今日" + averMsg + "20日平均线");
              if (this.averageCrossover && data.gztime != null) {
                ...
                if ((ma5averageStart <= ma10averageStart && ma10averageEnd <= ma5averageEnd) || (ma5averageStart >= ma10averageStart && ma10averageEnd >= ma5averageEnd))
                  this.monitorRemindMessage(data, rgbaList, "rgba(167, 176, 206,0.5)", "日平均线相交 5日X10日");
                if ((ma5averageStart <= ma20averageStart && ma20averageEnd <= ma5averageEnd) || (ma5averageStart >= ma20averageStart && ma20averageEnd >= ma5averageEnd))
                  this.monitorRemindMessage(data, rgbaList, "rgba(174 ,152, 255, 0.5)", "日平均线相交 5日X20日");//基金5日平均线和20日平均线相交
                if ((ma10averageStart <= ma20averageStart && ma20averageEnd <= ma10averageEnd) || (ma10averageStart >= ma20averageStart && ma20averageEnd >= ma10averageEnd))
                  this.monitorRemindMessage(data, rgbaList, "rgba(242 ,172, 158,0.5)", "日平均线相交 10日X20日");
              }
              let highestObj = {}, lowestObj = {};
              for (let i = 1; i < monitorFund.length; i++) if (monitorFund[i].DWJZ >= data.dwjz) if (monitorFund[i].JZZZL > 0) {
              ...
              if (this.peakedTips) {
                let str = "";
                if (highestObj.DWJZ == null && data.gszzl <= 0.5) str += "昨日高点，今日涨跌幅度过低，"; // 昨天为最高点,今天滞涨
                if (highestObj.DWJZ != null
                    && ((parseFloat(highestObj.DWJZ) + parseFloat(lowestObj.DWJZ)) / 2).toFixed(4) <= data.gsz
                    && data.gszzl <= 0.5 && data.gszzl >= -0.5) str += "超过最高点最低点的平均值，今日涨跌幅度过低，";
                if (str !== "") this.monitorRemindMessage(data, rgbaList, "rgba(78, 182,27,0.6)", str.slice(0, -1));
              }
              if (this.finallyTips) {
                let str = "";
                if (lowestObj.DWJZ == null && data.navchgrt <= -2.0 && data.gszzl <= 0.5 && data.gszzl >= -0.5)
                  str += "昨日暴跌，今日下跌幅度小，"; // 昨天暴跌,今天未持续
                let mindown = monitorFund.slice(0, 2).reduce((prev, cur) => prev + parseFloat(cur.JZZZL), 0) + parseFloat(data.gszzl);
                if (lowestObj.DWJZ == null && 1.5 >= mindown && mindown >= -1.5)
                  str += "连续多日小幅度下跌，今日" + (data.gszzl < 0 ? '下跌' : '上涨') + "，"; // 连续多日小幅度下跌
                let minimum = lowestObj.DWJZ == null ? monitorFund[0].DWJZ : lowestObj.DWJZ;
                let median = ((parseFloat(highestObj.DWJZ) + parseFloat(minimum)) / 2).toFixed(4);
                let scale = ((median - data.gsz) / median * 100);
                if (scale < 0 && scale >= -0.5 || scale > 0 && scale <= 0.5) str += "即将接近最高点最低点的平均值，";
                if (str !== "") this.monitorRemindMessage(data, rgbaList, "rgba(242,172,158,0.6)", str.slice(0, -1));
              }
            }
            dataList.push(data);
            for (let key in this.groupingArr2) {
              ...
              if (val.value.includes(data.fundcode)) {
                ...
                if (grouping == null)
                  groupingMap[val.name] = {name: val.name, label: val.label, closable: true, value: [data],};
                else
                  grouping.value.push(data);
              }
            }
          });
          if (this.showBadge === 1) if (this.BadgeContent === 2) {
          ...
          if (type === "add") {
            ...
            for (let grouping in groupingMap) {
              groupingMap[grouping].value = groupingMap[grouping].value.sort(this.compare(this.sortTypeObj.name, this.sortTypeObj.type));
            }
            this.groupingMap = groupingMap;
          } else {
            this.dataList = dataList;
    ...
    monitorRemindMessage(data, rgbaList, color, tips) {
      rgbaList.push(color);
      let scale = 20, style = '', str = '';
      if (data.gszzl < 0) style = 'background: rgba(78,182,27,0.2);background: linear-gradient(90deg, rgba(78,182,27,0.1) ' + scale + '%, rgba(78,182,27,0.1)' + (scale + 2) + '%,'
      else style = 'background: rgba(245,108,108,0.2);background: linear-gradient(90deg, rgba(245,108,108,0.1) ' + scale + '%, rgba(245,108,108,0.1)' + (scale + 2) + '%,'
      for (let i = 0; i < rgbaList.length; i++) str += rgbaList[i] + " " + (scale + 2 + i) + "%," + rgbaList[i] + " " + (scale + 2 + i + 1) + "%,";
      if (data.gszzl < 0) str = style + str + "rgba(78,182,27,0.2) " + (scale + 2 + rgbaList.length) + "%) !important;";
      else str = style + str + "rgba(245,108,108,0.2) " + (scale + 2 + rgbaList.length) + "%) !important;";
      data.trStyle = str;
    ...
    changeCost(item, ind) {
      debounce(() => {
        for (let fund of this.fundListM) {
          if (fund.code == item.fundcode) fund.cost = item.cost = this.numberReplace(item.cost);
        }
        chrome.storage.sync.set({fundListM: this.fundListM,}, () => {
              item.costGains = this.calculateCost(item);
    ...
    dragEnd(e) {
      this.$refs.menuDrag.style.bottom = (this.$refs.posmask.clientHeight - e.clientY - 30) + 'px';
      this.$refs.menuDrag.style.right = (this.$refs.posmask.clientWidth - e.clientX - 30) + 'px';
    },
    showmenu() {
      ...
      if (this.isEdit) this.optionMenuClassName(inputRowDisplay === 'none', this.$refs.gearDarkChange, 'gear-dark-change');
      this.optionMenuClassName(inputRowDisplay === 'none', this.$refs.menuDragIconRotate, 'menu-drag-icon-rotate');
      if (this.showCost || this.showGains) this.optionMenuClassName(inputRowDisplay === 'none', this.$refs.incomeCostGains, 'income-cost-gains');
      this.optionMenuClassName(inputRowDisplay === 'none', this.$refs.optionButton, 'menu-option');
      this.optionMenuClassName(inputRowDisplay === 'none', this.$refs.isEditButton, 'is-edit');
      this.optionMenuClassName(inputRowDisplay === 'none', this.$refs.earningsDetailButton, 'change-log');
      this.optionMenuClassName(inputRowDisplay === 'none', this.$refs.marketButton, 'market');
      if (!this.isDuringDate) {
        this.optionMenuClassName(inputRowDisplay === 'none', this.$refs.closedMarketButton, 'closed-market');
      } else {
        this.optionMenuClassName(inputRowDisplay === 'none', this.$refs.isLiveUpdateButton, 'closed-market');
      }
    },
    optionMenuClassName(display, el, className) {
      let elClassname = el.className;
      if (display) {
        if (elClassname.indexOf('close-' + className) >= 1) elClassname = elClassname.replace('close-' + className, 'open-' + className);
        else elClassname = elClassname + ' ' + 'open-' + className;
        el.className = elClassname;
        el.style.display = 'block';
      } else {
        if (elClassname.indexOf('open-' + className) >= 1) elClassname = elClassname.replace('open-' + className, 'close-' + className);
        else elClassname = elClassname + ' ' + 'close-' + className;
        el.className = elClassname;
      }
    },
    handleAnimationEnd(e) {
      if (e.animationName.indexOf('close-') > -1) e.target.style.display = 'none';
    },
    addTabPane() {
    ...
    removeTabPane(name) {
      ...
      this.$confirm('此操作将删除 \'' + groupingArr2.label + '\' 分组, 是否继续?', '提示', {
    ...
    tableColspan() {
      this.$nextTick(function () {
        ...
        if (this.$refs.tableColspanAll !== null) this.$refs.tableColspanAll.colSpan = childElementCount;
      });
    }
  },
};
...
.el-tabs--card > .el-tabs__header .el-tabs__item.is-active.is-closable .edit-tabs {
  width: 14px;
}
