...
import {beforeSendHeaders, finvestWeekDay, formatDateTime, funCurDate, iPhone_OS_13_2_3_like_Mac_OS_X, Windows_NT_10_0_Win64_x64} from '@/common/js/utils';
...
var setBadge = (fundcode, Realtime, type) => {
    let fundStr = null;
    if (type == 3) {
        let url = "https://push2.eastmoney.com/api/qt/ulist.np/get?fltt=2&fields=f3&secids=" + fundcode + "&_=" + new Date().getTime();
        ...
        // axios.get(url).then((res) => {
        axios.post("https://api.fund.eastmoney.com/favor/GetFundsInfo", {fcodes: fundStr}).then((res) => {
            let allAmount = 0;
            let allGains = 0;
            let textStr = null;
            let sumNum = 0;
            ...
            if (type == 1) {
                ...
                if (
                    ...
                    || isWeekDate() || checkHoliday(new Date())
                ) {
            ...
            let color = Realtime ? sumNum >= 0 ? "#F56C6C" : "#4eb61b" : "#4285f4";
            chrome.browserAction.setBadgeBackgroundColor({
                color: color
            });
        }).catch((error) => {
...
var monitorData = (url, timestamp, fund, monitorFund4, averageMa4) => {
    axios.get(url).then((res) => {
        ...
        for (let i = 0; i < res.data.TotalCount; i++) {
            ...
            var ma5total = ma5day.reduce((prev, cur) => prev + parseFloat(cur.DWJZ), 0);
            var ma5 = {date: ma5day[0].FSRQ, average: (ma5total / 5).toFixed(4)};
            ma5arr.push(ma5);
            var ma10day = maDatas.slice(i, 10 + i);
            var ma10total = ma10day.reduce((prev, cur) => prev + parseFloat(cur.DWJZ), 0);
            var ma10 = {date: ma10day[0].FSRQ, average: (ma10total / 10).toFixed(4)};
            ma10arr.push(ma10);
            var ma20day = maDatas.slice(i, 20 + i);
            var ma20total = ma20day.reduce((prev, cur) => prev + parseFloat(cur.DWJZ), 0);
            var ma20 = {date: ma20day[0].FSRQ, average: (ma20total / 20).toFixed(4)};
            ma20arr.push(ma20);
        }
        averageMa4.timestamp = timestamp;
        averageMa4[fund.code] = {"ma5arr": ma5arr, "ma10arr": ma10arr, "ma20arr": ma20arr,}
...
var getData = () => {
    ...
    chrome.webRequest.onBeforeSendHeaders.addListener(function (details) {
        if (details.url.indexOf('fundmobapi.eastmoney.com/FundMNewApi/FundMNInverstPosition') !== -1
            || details.url.indexOf('fundmobapi.eastmoney.com/FundMNewApi/FundMNFInfo') !== -1)
            iPhone_OS_13_2_3_like_Mac_OS_X(details);// 转换请求头为手机模式
        else Windows_NT_10_0_Win64_x64(details);
        if (details.url === 'https://api.fund.eastmoney.com/favor/GetFundsInfo')
            beforeSendHeaders(details, 'https://favor.fund.eastmoney.com', 'https://favor.fund.eastmoney.com/')
    ...
    chrome.storage.sync.get(["holiday", "fundListM", "RealtimeFundcode", "RealtimeIndcode", "showBadge", "BadgeContent", "BadgeType", "userId",], res => {
        chrome.storage.local.get(["positionArr2", "monitorFund4", "averageMa4"], reslocal => {
            ...
            axios.get(url).then((res) => {
                res.data.Datas.forEach((val) => {
                    ...
                    if (slt.fvest != null && slt.fvest.cvt) {
                        ...
                        if (!checkHoliday(new Date()) && !isWeekDate() && next <= date) {
                            let NAV = isNaN(val.NAV) ? null : val.NAV;
                            let addNum = (slt.fvest.addFundAmount / NAV).toFixed(2);
                            let num = slt.num ? slt.num : 0;
                            slt.num = (parseFloat(addNum) + parseFloat(num)).toFixed(2) + '';
                            slt.fvest.next = finvestWeekDay(slt.fvest.finvestWeek, slt.fvest.finvestDay);
                            ...
                            var obj = {
                                date: datestr,
                                type: "finfund", equity: NAV,
                                amount: parseFloat(slt.fvest.addFundAmount).toFixed(2),
                                quota: addNum, rates: slt.fvest.rates
                            };
            ...
            // 加载基础数据
            let timestamp = new Date(formatDateTime(new Date(), 'yyyy-MM-dd')).getTime();
            if (monitorFund4.timestamp == null || monitorFund4.timestamp < timestamp) {
                monitorFund4 = {}, averageMa4 = {};
                for (let i = 0; i < fundListM.length; i++) {
                    let val = fundListM[i];
                    let url1 = `https://fundmobapi.eastmoney.com/FundMApi/FundNetDiagram.ashx?FCODE=${val.code}&RANGE=3n&deviceid=Wap&plat=Wap&product=EFund&version=2.0.0&_=${new Date().getTime()}`;
                    monitorData(url1, timestamp, val, monitorFund4, averageMa4);
                }
            } else if (fundListM.filter((v) => monitorFund4[v.code] == null).length >= 1) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.type == "refreshBadgeAllGains") {
        ...
        let allGains = 0;
        let sumNum = 0;
        let datestr = formatDateTime(new Date(Number(request.data.Expansion.bjTime) * 1000), 'yyyy-MM-dd hh:mm');
        ...
        let color = isDuringDate() ? sumNum >= 0 ? "#F56C6C" : "#4eb61b" : "#4285f4";
        chrome.browserAction.setBadgeBackgroundColor({
            color: color
        });
    }
