<template>
  <div v-if="positionFinvestShow" class="shadow">
    ...
    >
      <p style="font-weight: bold;padding: 18px 20px;">
      ...
      <div>
        ...
        <div style="margin-bottom: 10px">
          ...
          <el-select
            ...
            @change="finvestWeek"
            v-model="finvestObj.finvestWeek"
            placeholder="请选择买入周期"
          >
          ...
          <el-select
            ...
            :disabled="finvestObj.finvestWeek == 3"
            @change="finvestWeekDay"
            v-model="finvestObj.finvestDay"
            placeholder="请选择买入日期"
          >
            <el-option
        ...
        <div style="margin-bottom: 10px">
          ...
          <input
            ...
            placeholder="请输入购买金额"
            @input="
              finvestObj.addFundAmount = numberReplace(finvestObj.addFundAmount)
            "
            v-model="finvestObj.addFundAmount"
            type="text"
          />
        </div>
        <div style="margin-bottom: 10px">
          ...
          <input
            ...
            placeholder="请输入手续费率"
            @input="
              finvestObj.addFundRate = numberReplace(finvestObj.addFundRate)
            "
            v-model="finvestObj.addFundRate"
            type="text"
          />
        </div>
        <div style="margin-bottom: 10px;position: relative;">
          ...
          <span class="nextDate" style="left: 270px;">{{
            finvestObj.count == null ? 0 : finvestObj.count
          }}</span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    finvestWeek(val) {
      ...
      } else if (val == 2) this.dateArr = this.monthday;
      this.finvestObj.finvestDay = null;
    },
    finvestWeekDay() {
      var nowDate = new Date();
      ...
      if (val == 3) {
        ...
        let weedDay = week.setDate(
          week.getDate() + parseInt(this.finvestObj.finvestDay)
        );
        this.finvestObj.next = formatDateTime(new Date(weedDay), "yyyy-MM-dd");
      } else if (val == 1) {
        let weedDay = week.setDate(
          week.getDate() + 7 + parseInt(this.finvestObj.finvestDay)
        );
        this.finvestObj.next = formatDateTime(new Date(weedDay), "yyyy-MM-dd");
      } else if (val == 2) {
        let month = new Date(nowDate.setDate(1));
        var monthNoeDayDate = new Date(month.setMonth(month.getMonth() + 1));
        var monthDay = monthNoeDayDate.setDate(
          monthNoeDayDate.getDate() + parseInt(this.finvestObj.finvestDay)
        );
