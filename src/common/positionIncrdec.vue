<template>
  <div v-if="positionIncrdecShow" class="shadow">
    <el-card class="reward-box" :class="darkMode ? 'container darkMode' : ''" :style="{ marginTop: top + 'px' }">
      <div slot="header" style="text-align: center;">
        <span style="font-weight: bold;">{{fund.name}} ({{fund.fundcode}})</span>
        <input class="btn" style="cursor: pointer;float: right;padding: 2px 5px;" type="button" value="x" @click="close"/>
      </div>
      <el-row>
        <el-form ref="form" :inline="true" :model="incrdecObj" label-width="40px">
          <el-col :sm="6" :xs="6">
          ...
          <span v-show="incrdecObj.checked == 'addfund'">
            <el-col :sm="6" :xs="6">
              <el-form-item label="日期">
                <el-select size="mini" style="width: 100px;" v-model="incrdecObj.addFundDate" placeholder="请选择买入日期">
            ...
            <el-col :sm="6" :xs="6">
              <el-form-item label="金额">
                <el-input-number size="mini" style="width: 100px;text-align: left;" v-model="incrdecObj.addFundAmount" :controls="false" :precision="2" :step="0.1" :max="999999999"></el-input-number>
              </el-form-item>
            </el-col>
            <el-col :sm="6" :xs="6">
              <el-form-item label="费率">
                <el-input style="width: 100px;" v-model="incrdecObj.addFundRate"></el-input>
              </el-form-item>
            </el-col>
          </span>
          <span v-show="incrdecObj.checked == 'decfund'">
            <el-col :sm="6" :xs="6">
              <el-form-item label="份额">
                <el-input-number style="width: 100px;" v-model="incrdecObj.decFundShare" :controls="false" :placeholder="'最大卖出份额:'+fund.num" :precision="2" :step="0.1" :max="parseFloat(fund.num)"></el-input-number>
              </el-form-item>
            </el-col>
            <el-col :sm="12" :xs="12">
              <el-form-item label="">
                <small style="color: rgba(0, 0, 0, 0.6);">{{'最大卖出份额:' + fund.num}}</small>
              </el-form-item>
            </el-col>
          </span>
        </el-form>
      </el-row>
      <el-table show-summary :summary-method="summaries" :default-sort="{prop: 'date', order: 'descending'}"  :data="tableData" :height="tableHeight" style="width: 100%;margin-bottom: 20px">
        <el-table-column sortable prop="date" label="买卖日期"></el-table-column>
        ...
        <el-table-column prop="rates" label="手续费额" >
          <template slot-scope="scope">
            {{(scope.row.amount * numberReplace(scope.row.rates) / 100).toFixed(2)}}
            <el-tag style="position: absolute;margin-left: 5px">{{scope.row.rates == '' ? '0.00%' : scope.row.rates}}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="equity" label="基金净值"></el-table-column>
        <el-table-column width="60" prop="type" label="类型" :filters="columnTypeFilters" :filter-method="filterHandler">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.type === 'addfund'" type="success" disable-transitions>买入</el-tag>
            <el-tag v-if="scope.row.type === 'decfund'" type="danger" disable-transitions>卖出</el-tag>
            <el-tag v-if="scope.row.type === 'finfund'" disable-transitions>定投</el-tag>
          </template>
        </el-table-column>
        <el-table-column width="60" prop="equity" label="">
          <template slot-scope="scope">
            <input class="btn" style="cursor: pointer;padding: 2px 5px;" type="button" value="x" @click="removePosArr(scope.row)"/>
          </template>
        </el-table-column>
      </el-table>
<!--      <small style="margin: 0 10px;color: rgba(0,0,0,0.6)"> 基金持有期是指卖出和买入期间间隔的自然日，节假日和周末都算在内，一般是按照申购确认日到赎回确认日的周期来算的。 </small>-->
      <div class="tab-row">
        <input class="btn success" style="cursor: pointer;" type="button" :value="'保存'+ (incrdecObj.checked == 'addfund'? '买入':'卖出')+'信息'" @click="saveFund"/>
...
<script>
import {ElCard, ElCol, ElForm, ElFormItem, ElInput, ElInputNumber, ElOption, ElRadioButton,
  ElRadioGroup, ElRow, ElSelect, ElTable, ElTableColumn, ElTag} from '@/common/js/commons';

export default {
  components: {
    ElRow, ElCol, ElFormItem, ElForm, ElSelect, ElInput,
    ElOption, ElInputNumber, ElTable, ElTableColumn,
    ElCard, ElRadioGroup, ElRadioButton, ElTag
  },
  props: {
    top: {
      type: Number,
  ...
  data() {
    return {
      ...
      tableData: [],
      tableHeight: 120,
      columnTypeFilters: [{text: '买入', value: 'addfund'}, {text: '卖出', value: 'decfund'}, {text: '定投', value: 'finfund'}],
    };
  },
  mounted() {
  ...
  methods: {
    init(val) {
      ...
      this.$axios.get(url).then((res) => {
        ...
        this.incrdecObj.addFundRate = this.infoData.RATE === '' ? "0.00%" : this.infoData.RATE;
      });
      ...
      chrome.storage.local.get(['positionArr2'], (res) => {
        var fundList = [];
        if (res.positionArr2 == null) chrome.storage.local.set({positionArr2: {}});
        else {
    ...
    saveFund() {
      var obj = {date: this.incrdecObj.addFundDate, type: this.incrdecObj.checked, equity: this.fund.dwjz}
      if (this.incrdecObj.checked === 'addfund' && this.incrdecObj.addFundAmount !== null && this.incrdecObj.addFundAmount !== '') {
        ...
        this.fund.amount = (parseFloat(this.fund.amount) + addAmount).toFixed(2) + ""
        obj.amount = addAmount.toFixed(2);
        obj.quota = (addAmount / this.fund.dwjz).toFixed(2);
        obj.rates = this.incrdecObj.addFundRate;
      } else if (this.incrdecObj.checked === 'decfund' && this.incrdecObj.decFundShare !== null && this.incrdecObj.decFundShare !== '') {
      ...
      this.$message({
        message: '保存' + (this.incrdecObj.checked === 'addfund' ? '买入' : '卖出') + '信息成功',
        type: "success",
        center: true,
      });
      // this.close();
    ...
    removePosArr(val) {
      chrome.storage.local.get(['positionArr2'], (res) => {
        ...
        if (val.type === "addfund" || val.type === "finfund") {
          var amount = (parseFloat(this.fund.amount) - parseFloat(val.amount)).toFixed(2);
          this.fund.amount = amount < 0 ? "0" : amount + "";
        } else if (val.type === "decfund") {
          this.fund.num = (parseFloat(this.fund.num) + parseFloat(val.quota)).toFixed(2) + "";
          checked = "decfund";
        }
    ...
    summaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (index === 3) {
          values = data.map(item => Number(item['amount'] * this.numberReplace(item[column.property]) / 100))
        }
