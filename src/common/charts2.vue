    ...
    <el-row>
      ...
      <el-col ref="table" :xs="8" v-if="this.chartType !== 'LJSY'">
        <el-table
          ...
          :default-sort="{ prop: 'FSRQ', order: 'descending' }"
          :data="dataTable"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span v-if="scope.row.JZZZL > 0" class="up"
                >{{ scope.row.JZZZL }}%</span
              >
...
export default {
  ...
  computed: {
    ...
    autoTableHeight() {
      ...
      if (this.chartType !== "LJSY" && boxWidth < 550) return 140;
      else return 255;
    }
  },
  mounted() {
          ...
          if (dataList) {
            ...
            this.option.tooltip.formatter = p => {
              return (
                ...
                '<a style="float: left;width: 60px;text-align: left">' +
                `${p[0].seriesName}` +
                '</a><a style="float: right;width: 60px">' +
                `${p[0].value}` +
                "</a><br/>" +
                '<a style="float: left;width: 60px;text-align: left">' +
                `${p[1].seriesName}` +
          ...
          chrome.storage.local.get(["positionArr2"], les => {
            chrome.runtime.sendMessage(
              ...
              response => {
                ...
                this.dataTable = dataList = res.data.Datas;
                let gztimeDate =
                  this.fund.gztime == null
                    ? this.fund.jzrq
                    : this.fund.gztime.substr(0, 10);
                let averageMa4 = this.averageMa4[this.fund.fundcode];
                let ma5average = [],
                  ma10average = [],
                  ma20average = [];
                let ma5arr = averageMa4.ma5arr,
                  ma10arr = averageMa4.ma10arr,
                  ma20arr = averageMa4.ma20arr;
                if (
                  ...
                  if (gztimeDate !== ma5arr[0].date) {
                    ...
                    let ma20day =
                      ...
                      20;
                    ma5arr.unshift({
                      average: ma5day.toFixed(4),
                      date: gztimeDate
                    });
                    ma10arr.unshift({
                      average: ma10day.toFixed(4),
                      date: gztimeDate
                    });
                    ma20arr.unshift({
                      average: ma20day.toFixed(4),
                      date: gztimeDate
                    });
                  }
                  if (gztimeDate !== dataList[dataList.length - 1].FSRQ) {
                    ...
                    let data = {
                      FSRQ: gztimeDate,
                      DWJZ: gsz,
                      LJJZ: (parseFloat(monitorFund4.LJJZ) + differ).toFixed(4),
                      JZZZL: this.fund.gszzl
                    };
                    dataList.push(data);
                  }
                }
                ...
                if (les.positionArr2 != null) {
                  fundList =
                    les.positionArr2[this.fund.fundcode] != null
                      ? les.positionArr2[this.fund.fundcode]
                      : [];
                  if (fundList.length !== 0)
                    for (let i = 0; i < dataList.length; i++) {
                      let jc = fundList.filter(
                        item => item.date === dataList[i].FSRQ
                      );
                      if (jc.length !== 0) {
                        let color = "",
                          textValue = "";
                        for (let j = 0; j < jc.length; j++) {
                          if (
                            jc[j].type === "finfund" &&
                            this.sltAmountRange.indexOf("fin") !== -1
                          ) {
                            color = "#4285f4";
                            textValue = "定";
                          }
                          if (
                            jc[j].type === "addfund" &&
                            this.sltAmountRange.indexOf("add") !== -1
                          ) {
                            color = "#4eb61b";
                            textValue = "买";
                          }
                          if (
                            jc[j].type === "decfund" &&
                            this.sltAmountRange.indexOf("dec") !== -1
                          ) {
                            color = "#F56C6C";
                            textValue = "卖";
                          }
                ...
                if (
                  this.sltTimeRange === "y" ||
                  this.sltTimeRange === "3y" ||
                  this.sltTimeRange === "n" ||
                  this.sltTimeRange === "6y"
                ) {
                  let mp5 = {
                    type: "line",
                    name: "MA5",
                ...
                this.option.tooltip = {
                  ...
                  formatter: function(p) {
                    let jc = fundList.filter(
                      item => item.date === dataList[p[0].dataIndex].FSRQ
                    );
                    let tooltip =
                      "" +
                      `${p[0].name}` +
                      '<hr size=1 style="margin: 3px 0">' +
                      '<a style="float: left;width: 60px;text-align: left">单位净值</a><a style="float: right;width: 60px">' +
                      `${p[0].value}` +
                      "</a><br/>" +
                      '<a style="float: left;width: 60px;text-align: left">累计净值</a><a style="float: right;width: 60px">' +
                      `${p[1].value}` +
                      "</a><br/>" +
                      '<a style="float: left;width: 60px;text-align: left">日增长率</a><a style="float: right;width: 60px">' +
                      `${dataList[p[0].dataIndex].JZZZL}%` +
                      "</a><br/>";
                    if (jc.length !== 0)
                      for (let i = 0; i < jc.length; i++) {
                        ...
                        if (
                          jc[i].type === "finfund" &&
                          _this.sltAmountRange.indexOf("fin") !== -1
                        )
                          textValue = "定投";
                        if (
                          jc[i].type === "addfund" &&
                          _this.sltAmountRange.indexOf("add") !== -1
                        )
                          textValue = "买入";
                        if (
                          jc[i].type === "decfund" &&
                          _this.sltAmountRange.indexOf("dec") !== -1
                        )
                          textValue = "卖出";
                        if (textValue !== "")
                          tooltip +=
